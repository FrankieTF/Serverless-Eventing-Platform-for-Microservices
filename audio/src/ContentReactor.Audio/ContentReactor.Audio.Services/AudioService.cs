...
namespace ContentReactor.Audio.Services
{
    ...
    public class AudioService : IAudioService
    {
        ...
        public (string id, string url) BeginAddAudioNote(string userId)
        {
            ...
            var blob = BlobHelper.GetBlobClient(AudioBlobContainerName, audioId);


            var urlAndSas = BlobHelper.GetSasUriForBlob(blob, BlobSasPermissions.Create | BlobSasPermissions.Write);
        ...
        public async Task<AudioNoteDetails> GetAudioNoteAsync(string id, string userId)
        {
            ...
            var audioUrl = BlobHelper.GetSasUriForBlob(audioBlob, BlobSasPermissions.Read);
        ...
        {
            var blobs = await BlobHelper.ListBlobsAsync(AudioBlobContainerName);
        ...
        {
            // delete the blog
            var blob = BlobHelper.GetBlobClient(AudioBlobContainerName, id);
        ...
        {
            // get the blob
            var audioBlob = BlobHelper.GetBlobClient(AudioBlobContainerName, id);
            if (audioBlob == null)
            {
                return null;
            }
